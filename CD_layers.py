...
def CD_leaky_relu(inputs: Tensor, negative_slope: float = 0.01, inplace: bool = False) -> Tensor:
    ...
    if isinstance(inputs, dict):
        ...
        result['irrel'] = F.leaky_relu(rel + irrel, negative_slope, inplace) - F.leaky_relu(rel, negative_slope, inplace)
...
def CD_softmax(src: Tensor, index: Optional[Tensor], ptr: Optional[Tensor] = None,
    ...
    if cd_explain:
        return {'rel': (out/(out_sum + 1e-16)) * _rel, 'irrel': (out/(out_sum + 1e-16)) - (out/(out_sum + 1e-16)) * _rel}
    else:
...
class CD_GATConv(MessagePassing):
    ...
    def message(self, x_j: Tensor, alpha_j: Tensor, alpha_i: OptTensor,
        ...
        if not cd_explain:
            ...
            result['irrel'] = x_irrel_j * alpha_dict['rel'].unsqueeze(-1) +  x_irrel_j * alpha_dict['irrel'].unsqueeze(-1) + x_rel_j * alpha_dict['irrel'].unsqueeze(-1)
