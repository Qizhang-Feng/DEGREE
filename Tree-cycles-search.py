...
for _ in range(1):
    ...
    node_pred_task_train(gcn_model, dataset, max_epoch=150, lr=0.005, temp_name='cycles_temp', train_rate = 0.80)
    ...
    for idx in dataset.allnodes:
        ...
        #break
    import pickle
    with open('time_cycles_gat', 'wb') as f:
        pickle.dump({'all_elapsed': all_elapsed, 'all_len':all_len}, f)
    torch.save(load_model, './Tree_cycles_search_checkpoint_GAT_3/acc_' + str(np.mean(acc_list)) + '_auc_' + str(np.mean(auc_list)))