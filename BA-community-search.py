...
for i in range(1):
    ...
    node_pred_task_train(gcn_model, dataset, max_epoch=150, lr=0.005, temp_name='community_temp', train_rate = 0.8)
    ...
    for idx in dataset.allnodes:
        ...
        #break
    import pickle
    with open('time_community_gat', 'wb') as f:
        pickle.dump({'all_elapsed': all_elapsed, 'all_len':all_len}, f)
    torch.save(load_model, './BA_community_search_checkpoint_GAT_3/acc_' + str(np.mean(acc_list)) + '_auc_' + str(np.mean(auc_list)))